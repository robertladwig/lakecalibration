...
% commented out all water quality parameters for calibration!
...
%% get simulated data, test-case specific
d1=dlmread('DT_1.csv',',',['B2..C8425']);
d3=dlmread('DT_3.csv',',',['B2..C8425']);
d5=dlmread('DT_5.csv',',',['B2..C8425']);
d7=dlmread('DT_7.csv',',',['B2..C8425']);
d8=dlmread('DT_8.csv',',',['B2..C8425']);
d9=dlmread('DT_9.csv',',',['B2..C8425']);
d10=dlmread('DT_10.csv',',',['B2..C8425']);
d13=dlmread('DT_13.csv',',',['B2..C8425']);
d14=dlmread('DT_14.csv',',',['B2..C8425']);
d_csv=[d1,d3,d5,d7,d8,d9,d10,d13,d14];
...
dep_csv=[1 3 5 7 8 9 10 13 14]; % lake-specific (field-specific) depths
aParam=diff(isParam);
simParam=zeros(aParam(1),length(isParam)+1);
field_depParam=15.43-measParam(1:15,2);
...
fieldParam(:,1)=measParam(1:15,3);
for i=2:length(isParam)
    fieldParam(:,i)=measParam(isParam(i-1):(isParam(i-1)+14),3);

    if i==length(isParam)
        fieldParam(:,i+1)=measParam(isParam(i):(isParam(i)+14),3);
